...
type Application = {id?: number;name?: string;email?: string;field_of_work?: string;role?: string;role_other?: string;hear_about?: string;tech_1_year?: string;tech_2_year?: string;tech_5_year?: string;dietary?: string;dietary_other?: string;accessibility?: string;recording_consent?: string;anything_else?: string;};

type DetailProps = {label: string;value: string | number | null | undefined;};

function Detail({label,value,}: DetailProps) {return ({label}
...
export default function AdminPage() {const [applications, setApplications] =useState<Application[]>([]);const [loading, setLoading] = useState(true);const [error, setError] = useState("");const [search, setSearch] = useState("");

const loadApplications = async () => {setLoading(true);setError("");
...
const exportCsv = () => {const columns = ["id","name","email","field_of_work","role","role_other","hear_about","tech_1_year","tech_2_year","tech_5_year","dietary","dietary_other","accessibility","recording_consent","anything_else",];
...
const visible = applications.filter((a: Application) => {const q = search.trim().toLowerCase();
